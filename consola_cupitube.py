...
def ejecutar_buscar_cupitubers_por_pais_categoria_monetizacion(cupitube: dict) -> None:
    ...
    monetizacion = input("Ingrese el tipo de monetización que desea buscar: ").title()
    
    pais_encontrado = None
    
    for pais_key in cupitube.keys():
        
        if pais_key.lower() == pais.lower():
            
            pais_encontrado = pais_key
        
    if pais_encontrado:
        
        cupitubers = ct.buscar_cupitubers_por_pais_categoria_monetizacion(cupitube, pais_encontrado, categoria, monetizacion)

    if cupitubers != []:
        print("Los Cupitubers de", pais_encontrado, "que pertenecen a la categoría", categoria, "y tienen monetización", monetizacion, "son \n", cupitubers)
...
def ejecutar_buscar_cupituber_mas_antiguo(cupitube: dict) -> None:
    ...
    for country in cupitube:  
        ...
        for cada_cupituber in cupitube[country]:
            ...
            if cada_cupituber["started"] == cupituber["started"]:
                
                fecha = cada_cupituber["started"]
    
    
    print ("El CupiTuber más antiguo es", cupituber["cupituber"], "y empezó en", fecha, "." )
...
def ejecutar_obtener_visitas_por_categoria(cupitube: dict) -> None:
    ...
    for country in cupitube:  
        ...
        for cada_cupituber in cupitube[country]:
            ...
            if cada_cupituber["category"].lower() == category:
                
                categoria = cada_cupituber["category"]
                
    vist_cat = ct.obtener_visitas_por_categoria(cupitube, categoria)
    
    if categoria == None:
        print("Lo sentimos, categoría no encontrada.")
        
    else:
        print ("El total de visitas para la categoría", categoria, "es:", vist_cat, ".")
...
def ejecutar_obtener_categoria_con_mas_visitas(cupitube: dict) -> None:
    ...
    categoria = ct.obtener_categoria_con_mas_visitas(cupitube)

    
    print ("La categoría con más visitas es", categoria["Categoria"], "con" , categoria["Visitas"], "visitas.")
...
def ejecutar_recomendar_cupituber(cupitube: dict) -> None:
    ...
    sub_min = int(input("Ingrese la cantidad minima de suscriptores: "))
    sub_max = int(input("Ingrese la cantidad máxima de suscriptores: "))
    

    ### strip and especificar. 
    fech_min = input("Ingrese la fecha miníma de búsqueda en formato YYYY-MM-DD: ").strip()
    fech_max = input("Ingrese la fecha máxima de búsqueda en formato YYYY-MM-DD: ").strip()
    vid_minimos = int(input("Ingrese la cantidad mínima de videos requeridos: "))
    pal_clave = input("Ingrese la palabra clave que desea buscar: ").strip()
    
    cupituber = ct.recomendar_cupituber(cupitube, sub_min, sub_max, fech_min, fech_max, vid_minimos, pal_clave)
    
    if cupituber == {} or pal_clave == "":
